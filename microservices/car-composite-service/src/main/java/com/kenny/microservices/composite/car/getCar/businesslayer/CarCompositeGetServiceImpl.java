...
@Service
public class CarCompositeGetServiceImpl implements CarCompositeGetService {
    private static final Logger LOG = LoggerFactory.getLogger(CarCompositeGetServiceImpl.class);

    private final CarCompositeGetIntegration integration;
    private final ServiceUtil serviceUtil;

    public CarCompositeGetServiceImpl(CarCompositeGetIntegration integration, ServiceUtil serviceUtil){
    ...
    public CarAggregate getCarModel(int carId) {
        CarModel carModel = integration.getCarModel(carId);
        if(carModel== null) throw new NotFoundException("No cars found for carId: " + carId);

        List<BuildPrice> buildPrices = integration.getBuildPrice(carId);
        List<Customer> customers = integration.getCustomer(carId);

        return createCarAggregate(carModel,buildPrices,customers, serviceUtil.getServiceAddress());
    }
